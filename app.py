...
# 2. ESTILOS VISUALES EN CSS (Para crear las tarjetas redondeadas)
st.markdown("""
    <style>
...
# 3. VALORES INICIALES DE LAS TASAS
...
# Tasa Máster de Control Interno
st.session_state.referencia_master = st.session_state.tasa_binance

# 4. PANEL IZQUIERDO (Sidebar de Control)
...
# Input manual para modificar Binance
nueva_tasa = st.sidebar.number_input("Tasa Binance P2P (Manual):", min_value=0.0, value=st.session_state.tasa_binance, step=0.1)
...
# Menú Completo del Panel Izquierdo
modulos = [
...
# 5. DISEÑO DE BLOQUES CENTRAL Y DERECHO
if modulo_seleccionado == "📊 Panel Principal / Dashboard":
    ...
    with col_derecha:
        st.markdown('<div class="card-white" style="background-color: #EFF6FF; border: 1px solid #BFDBFE;"><div class="card-title" style="color: #1E40AF;">🛒 MÓDULO OPERATIVO</div><p style="font-size:12px; color: #1E3A8A;">Este panel se activa al facturar o cotizar.</p></div>', unsafe_allow_html=True)
else:
